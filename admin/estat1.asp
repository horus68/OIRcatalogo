...
<script type="text/javascript">
	...
	if (flag==false) {document.getElementById("chart_div").innerHTML="<div style='position: relative; background: url(../imagens/picgraficos/no-graph.png); width: 385px; height: 296px;'><div style='position: absolute; top: 1em; lefght: 2em;padding: 4px;color:red;'><b>Não há dados para mostrar!</b></div></div>";return}    
...
<% end if%>
	...
	var flag=false;
	for(i=0;i< t.length-1;i++) if (t[i]!=0) flag=true;
	if (flag==false) {document.getElementById("chart_div1").innerHTML="<div style='position: relative; background: url(../imagens/picgraficos/no-graph.png); width: 385px; height: 296px;'><div style='position: absolute; top: 1em; lefght: 2em;padding: 4px;color:red;'><b>Não há dados para mostrar!</b></div></div>";return}    
	var data = new google.visualization.DataTable();     
	data.addColumn('string', 'Instituições');        
...
function drawChart2() {
	...
	var flag=false;
	for(i=0;i< t.length-1;i++) if (t[i]!=0) flag=true;
	if (flag==false) {document.getElementById("chart_div2").innerHTML="<div style='position: relative; background: url(../imagens/picgraficos/no-graph.png); width: 385px; height: 296px;'><div style='position: absolute; top: 1em; lefght: 2em;padding: 4px;color:red;'><b>Não há dados para mostrar!</b></div></div>";return}    
	var data = new google.visualization.DataTable();     
	data.addColumn('string', 'Instituições');        
	data.addColumn('number', 'Totais'); 
	data.addColumn('number', 'Confirmados');        
...
<%if valores(6)="S" then %> 
		...
		<% end if%>
chart.draw(data, {max: <%=maxi%>, width: 575, height: 296, enableTootlTip: true,  axisFontSize: 11, is3D: true, colors:[{color:'red', darker:'gray'} ,{color:'cyan', darker:'gray'}], title: ''});
...
<div id="admbotoes" style="float:right"><a href="admin.asp?id=2&mnres=2"><img src="../imagens/picactions/icon_close.gif" border=0 alt="Voltar à página anterior"></a><% if flag=1 then %><a href="javascript:window.print()"><img src="../imagens/picactions/icon_imprimir.gif" border=0 alt="Imprimir esta página" title="Imprimir esta página"></a><%end if%>
	...
	<div id="painel">
		...
		<br />
			<b>B - Pedidos totais vs confirmados: [Empréstimos Interbibliotecas]</b>
			<table>
			<tr>
